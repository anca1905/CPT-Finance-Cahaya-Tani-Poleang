...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['simpan_jurnal'])) {
    ...
    if (count($kode_akuns) < 2) {
        ...
        try {
            $conn->query("INSERT INTO tb_jurnal_umum (no_referensi, tanggal, deskripsi, total_debit, total_kredit, jenis_jurnal) VALUES ('$no_referensi', '$tanggal', '$deskripsi', $total_debit, $total_kredit, 'Penyesuaian')");
            ...
            $conn->commit();
            echo "<script>alert('Jurnal Penyesuaian Berhasil Disimpan!'); window.location.href='jurnal_penyesuaian.php';</script>";
            exit;
        } catch (Exception $e) {
            $conn->rollback();
...
$total_jurnal_q = $conn->query("SELECT COUNT(*) as t FROM tb_jurnal_umum WHERE jenis_jurnal = 'Penyesuaian'");
$total_jurnal = $total_jurnal_q->fetch_assoc()['t'];
$total_pages = ceil($total_jurnal / $limit);

$jurnal_umum = $conn->query("SELECT * FROM tb_jurnal_umum WHERE jenis_jurnal = 'Penyesuaian' ORDER BY tanggal DESC, id DESC LIMIT $limit OFFSET $offset");
?>

<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0 text-gray-800">Jurnal Penyesuaian</h1>
    <button class="btn btn-primary shadow-sm" data-toggle="modal" data-target="#modalJurnal">
        <i class="fas fa-plus fa-sm text-white-50"></i> Tambah Jurnal Penyesuaian
    </button>
</div>
...
<div class="card shadow mb-4">
    <div class="card-header py-3">
        <h6 class="m-0 font-weight-bold text-primary">Riwayat Jurnal Penyesuaian</h6>
    </div>
    <div class="card-body">
        <div class="table-responsive">
            <table class="table table-bordered table-sm" width="100%" cellspacing="0">
                ...
                <tbody>
                    ...
                    <tr><td colspan="6" class="text-center">Belum ada transaksi di jurnal penyesuaian</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
        ...
        <form method="POST" id="formJurnal">
            <div class="modal-content">
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title">Input Jurnal Penyesuaian</h5>
                ...
                <div class="modal-body">
                    <div class="form-row">
                        ...
                        <div class="form-group col-md-9">
                            <label>Keterangan Jurnal Penyesuaian / Deskripsi</label>
                            <input type="text" class="form-control" name="deskripsi" placeholder="Contoh: Penyesuaian Beban Perlengkapan Bulan Ini" required>
                        </div>
                    </div>
